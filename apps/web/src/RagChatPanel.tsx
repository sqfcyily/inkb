...
export const RagChatPanel: React.FC<{
  ...
  return (
    <>
      <div 
        className={`mac-glass flex flex-col shrink-0 bg-[var(--sidebar-bg)] transition-all duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${
          isOpen ? 'w-80 lg:w-96 opacity-100 border-l border-[var(--border)]' : 'w-0 opacity-0 overflow-hidden !border-none'
        }`}
      >
        
        <div className="mac-glass h-15 flex items-center justify-between px-4 border-b border-[var(--border)] shrink-0 bg-[var(--toolbar-bg)]">
          <div className="font-semibold text-[15px] tracking-tight text-[var(--text)]">Knowledge</div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-[var(--muted)] space-y-3">
              ...
              <p className="text-sm">Ask about your notes</p>
            </div>
          ) : (
            messages.map((m: any) => (
              <div key={m.id} className="flex flex-col gap-1">
                <div className="px-1 text-[11px] font-semibold tracking-wider uppercase text-[var(--muted)]">
                  {m.role === 'user' ? 'You' : 'AI'}
                </div>
                <div
                  className={`w-full rounded-xl border px-3.5 py-3 text-[14px] leading-relaxed shadow-[0_1px_0_rgba(0,0,0,0.04)] ${
                    m.role === 'user'
                      ? 'bg-[color-mix(in_oklab,var(--sk-focus-color)_10%,transparent)] border-[color-mix(in_oklab,var(--sk-focus-color)_22%,var(--border))] text-[var(--text)]'
                      : 'bg-[color-mix(in_oklab,var(--editor-bg)_70%,transparent)] border-[var(--border)] text-[var(--text)]'
                  }`}
                >
                  {m.content ? (
                    m.role === 'assistant' ? (
        ...
        <div className="mac-glass p-4 shrink-0 border-t border-[var(--border)] bg-[var(--toolbar-bg)]">
          {mode === 'doc' && attachedNote ? (
            <div className="mb-2">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-[var(--panel-bg)] text-[var(--text)] rounded-lg border border-[var(--border)] text-xs font-medium max-w-full">
                ...
                <button
                  ...
                  className="ml-1 p-0.5 rounded-full hover:bg-[var(--panel-bg-hover)] transition-colors shrink-0"
                  title="Cancel"
                >
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                    <path d="M2 10L10 2M2 2L10 10" />
